...
const safeNum = (v, fallback = 0) => { const n = Number(v); return isFinite(n) && n >= 0 ? Math.round(n) : fallback; };

/* ─── MACRO CALC ─── */
function calcMacros(goalType, kcal, weight) {
  const w = Math.max(parseFloat(weight) || 70, 30);
  const k = Math.max(safeNum(kcal, 2000), 800);
  const protG = goalType === "perdita" ? w * 2.2 : goalType === "massa" ? w * 2.0 : w * 1.8;
  const fatG  = goalType === "perdita" ? k * 0.25 / 9 : goalType === "massa" ? k * 0.30 / 9 : k * 0.28 / 9;
  const carbG = Math.max((k - protG * 4 - fatG * 9) / 4, 50);
  return { p: Math.round(protG), c: Math.round(carbG), f: Math.round(fatG) };
}
...
function migrateFromLegacy() {
  try {
    ...
    // --- profile ---
    const oldProfile = JSON.parse(localStorage.getItem("nutriai_profile"));
    if (oldProfile?.setup) {
      const macros = oldProfile.macros || calcMacros(oldProfile.goalType || "mantenimento", oldProfile.goal || 2000, oldProfile.weight || 70);
      const newProfile = {
        name: oldProfile.name || "",
        kcal: oldProfile.goal || oldProfile.calories || 2000,
        macros,
        goalType: oldProfile.goalType || "mantenimento",
        weight: oldProfile.weight || 70,
        setup: true,
      };
      localStorage.setItem("nutriai_v3_profile", JSON.stringify(newProfile));
    }

    // --- meals (old format: one object {dateString: [meals]}) ---
    const oldMeals = JSON.parse(localStorage.getItem("nutriai_meals"));
    if (oldMeals && typeof oldMeals === "object") {
      Object.entries(oldMeals).forEach(([day, meals]) => {
...
function saveProfile(p) { try { localStorage.setItem("nutriai_v3_profile", JSON.stringify(p)); } catch {} }
function loadMeals(key) { try { return JSON.parse(localStorage.getItem("nutriai_v3_meals_" + key)) || []; } catch { return []; } }
function saveMeals(key, meals) { try { localStorage.setItem("nutriai_v3_meals_" + key, JSON.stringify(meals)); } catch {} }
function loadHistory() {
  try {
    const keys = Object.keys(localStorage).filter(k => k.startsWith("nutriai_v3_meals_"));
    const out = {};
    keys.forEach(k => { const day = k.replace("nutriai_v3_meals_",""); out[day] = JSON.parse(localStorage.getItem(k)) || []; });
    return out;
  } catch { return {}; }
}
...
function GradBtn({ children, onClick, disabled, style = {} }) {
  ...
  return (
    <button onClick={onClick} disabled={disabled}
      ...
      style={{
        background: disabled ? C.dim : hover
          ? `linear-gradient(135deg,#5af0e8,#b89fff)` : `linear-gradient(135deg,${C.teal},${C.purple})`,
...
function Ring({ eaten, goal, size = 180, strokeW = 10, color = C.teal, label = "Rimanenti", unit = "kcal" }) {
  ...
  const dash = pct * circ;
  const id = `glow${color.replace(/[^a-z0-9]/gi,"")}${size}`;
  const remaining = goal > 0 ? Math.max(goal - eaten, 0) : 0;
  const displayVal = over ? `+${eaten - goal}` : remaining;

  return (
    <div style={{ position:"relative", width:size, height:size, flexShrink:0 }}>
      <svg width={size} height={size} style={{ display:"block" }}>
        <defs>
          <filter id={id}><feGaussianBlur stdDeviation="2.5" result="b"/><feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge></filter>
        </defs>
        ...
        {[0,25,50,75].map(p => {
          ...
          return <line key={p} x1={cx+r*Math.cos(a)} y1={cy+r*Math.sin(a)} x2={cx+(r+5)*Math.cos(a)} y2={cy+(r+5)*Math.sin(a)} stroke={C.dim} strokeWidth={1.5}/>;
        })}
      </svg>
      <div style={{ position:"absolute", inset:0, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", pointerEvents:"none" }}>
        ...
        <span style={{ fontSize: size > 120 ? 32 : 20, fontWeight:800, color: over ? C.red : C.text, fontFamily:"'DM Mono',monospace", lineHeight:1.1, marginTop:2 }}>{displayVal}</span>
...
/* ─── MACRO RING CARD ─── */
function MacroCard({ label, eaten, goal, color, icon, unit = "g" }) {
  const pct = safePct(eaten, goal);
  const over = goal > 0 && eaten > goal;
  ...
  return (
    <div style={{ background:`${color}0c`, border:`1px solid ${color}22`, borderRadius:18, padding:"16px 12px", display:"flex", flexDirection:"column", alignItems:"center", gap:8, flex:"1 1 0" }}>
      <span style={{ fontSize:22 }}>{icon}</span>
      <Ring eaten={safeNum(eaten)} goal={safeNum(goal)} size={88} strokeW={7} color={color} label="left" unit={unit}/>
      <div style={{ textAlign:"center", width:"100%" }}>
        <div style={{ fontSize:11, color:C.muted, textTransform:"uppercase", letterSpacing:".08em", marginBottom:4 }}>{label}</div>
        <div style={{ background:C.dim, borderRadius:6, height:5, overflow:"hidden" }}>
          <div style={{ width:barW, height:"100%", background:col, borderRadius:6, transition:"width .6s" }}/>
        </div>
        <div style={{ display:"flex", justifyContent:"space-between", marginTop:4 }}>
          <span style={{ fontSize:10, color:C.dim }}>{eaten}{unit}</span>
          <span style={{ fontSize:10, color:over?C.red:col, fontWeight:700 }}>
            {over ? `+${eaten-goal}${unit}` : `${remaining}${unit} left`}
          </span>
        </div>
      </div>
    </div>
  ...
  return (
    <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:18, padding:"15px 17px", display:"flex", gap:13, alignItems:"flex-start", position:"relative", overflow:"hidden" }}>
      ...
      <div style={{ flex:1, minWidth:0 }}>
        ...
        {meal.note && <p style={{ margin:"7px 0 0", fontSize:12, color:C.muted, fontStyle:"italic", lineHeight:1.35 }}>💡 {meal.note}</p>}
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:20, padding:"16px 18px" }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12 }}>
        <span style={{ fontSize:11, color:C.muted, textTransform:"uppercase", letterSpacing:".1em" }}>💧 Acqua giornaliera</span>
      ...
      <div style={{ display:"flex", gap:6, flexWrap:"wrap", marginBottom:10 }}>
        {Array.from({length:goal}).map((_,i) => (
          <button key={i} onClick={() => setGlasses(i < glasses ? i : i + 1)} style={{
            width:34, height:34, borderRadius:10, border:`1px solid ${i < glasses ? "#3de8df55" : C.border}`,
            background: i < glasses ? "#3de8df18" : C.dim, cursor:"pointer",
            fontSize:16, transition:"all .2s", display:"flex", alignItems:"center", justifyContent:"center",
          }}>💧</button>
        ))}
      </div>
      <div style={{ background:C.dim, borderRadius:6, height:5, overflow:"hidden" }}>
...
const QUICK_ADDS = {
  ...
  pranzo:    ["Pasta al pomodoro 180g","Insalata di pollo","Riso con verdure","Panino con prosciutto","Zuppa di legumi"],
  cena:      ["Salmone alla griglia 150g","Petto di pollo 200g","Minestrone","Bistecca 200g","Pesce al forno"],
...
function SetupScreen({ onSave }) {
  ...
  const steps=[
    ...
    <div key="s2" style={{display:"flex",flexDirection:"column",gap:12}}>
      ...
      {weight&&height&&age&&(
        <div style={{background:`${C.teal}0a`,border:`1px solid ${C.teal}25`,borderRadius:14,padding:"14px",marginTop:4}}>
...
export default function NutriAI() {
  ...
  /* persist meals */
  useEffect(() => { if(profile) saveMeals(todayKey(), meals); }, [meals]);
  useEffect(() => { try{ localStorage.setItem("nutriai_water_"+todayKey(), glasses) }catch{} }, [glasses]);

  const showToast = (msg, type="success") => { setToast({msg,type}); setTimeout(()=>setToast(null),2800); };

  /* totals — always safe numbers */
  const totals = meals.reduce((a,m) => ({
    cal: a.cal + safeNum(m.calories),
    p:   a.p   + safeNum(m.protein),
    c:   a.c   + safeNum(m.carbs),
    f:   a.f   + safeNum(m.fat),
  }), {cal:0,p:0,c:0,f:0});
  ...
  const analyzeMeal = useCallback(async (text = input, retry = 0) => {
    if (!text.trim()) return;
    setLoading(true);
    const prompt = `Sei un nutrizionista esperto. Analizza questo pasto e stima i valori nutrizionali medi.
Pasto: "${text}"
Tipo: ${mealType}

REGOLE:
- Stima porzioni medie italiane se non specificate
- Rispondi ESCLUSIVAMENTE con JSON valido, senza testo aggiuntivo, senza markdown
- Tutti i valori devono essere numeri interi positivi
- "note" deve essere una stringa breve in italiano (max 80 caratteri)
...
{"calories":0,"protein":0,"carbs":0,"fat":0,"note":""}`;
    ...
    try {
      ...
      const cal  = safeNum(parsed.calories, 0);
      const prot = safeNum(parsed.protein,  0);
      const carb = safeNum(parsed.carbs,    0);
      const fat  = safeNum(parsed.fat,      0);

      if (cal === 0 && retry < 2) { return analyzeMeal(text, retry + 1); }

      setMeals(prev => [...prev, {
        id: Date.now(), description: text.trim(), mealType,
      ...
      if (retry < 2) {
        ...
        return analyzeMeal(text, retry + 1);
      }
  ...
  const deleteMeal = useCallback(id => {
    ...
    showToast("🗑️ Pasto eliminato");
  }, []);

  const saveProfile = (p) => { const np={...p,setup:true}; setProfile(np); saveProfile(np); };
  // fix naming conflict
  const handleSaveProfile = (p) => { const np={...p,setup:true}; setProfile(np); try{localStorage.setItem("nutriai_v3_profile",JSON.stringify(np))}catch{} };

  if (!profile?.setup) return <SetupScreen onSave={handleSaveProfile}/>;

  const history = loadHistory();
  const histDays = Object.entries(history)
    .filter(([d]) => d !== todayKey())
  ...
  const streak = (() => {
    ...
    while(true){ const k=d.toDateString(); const ms=history[k]; if(!ms||ms.length===0) break; s++; d.setDate(d.getDate()-1); }
    return s;
  })();
    ...
    <div style={{minHeight:"100vh",background:C.bg,color:C.text,fontFamily:"'Inter','DM Sans',sans-serif",paddingBottom:80}}>
      ...
      {toast && (
        <div style={{
          ...
          background: toast.type==="error" ? "#2a1015" : "#0f2a20",
      ...
      <div style={{maxWidth:580,margin:"0 auto",padding:"18px 15px"}}>
        {tab==="oggi" && <>
          ...
          <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:22,padding:"22px 18px",marginBottom:10,display:"flex",flexDirection:"column",alignItems:"center",gap:6}}>
            ...
            <div style={{display:"flex",gap:16,marginTop:4}}>
              <div style={{textAlign:"center"}}>
                <div style={{fontSize:10,color:C.dim}}>Colazione</div>
                <div style={{fontSize:12,color:C.amber,fontFamily:"'DM Mono',monospace",fontWeight:700}}>{meals.filter(m=>m.mealType==="colazione").reduce((s,m)=>s+safeNum(m.calories),0)} kcal</div>
              </div>
              <div style={{textAlign:"center"}}>
                <div style={{fontSize:10,color:C.dim}}>Pranzo</div>
                <div style={{fontSize:12,color:C.teal,fontFamily:"'DM Mono',monospace",fontWeight:700}}>{meals.filter(m=>m.mealType==="pranzo").reduce((s,m)=>s+safeNum(m.calories),0)} kcal</div>
              </div>
              <div style={{textAlign:"center"}}>
                <div style={{fontSize:10,color:C.dim}}>Cena</div>
                <div style={{fontSize:12,color:C.purple,fontFamily:"'DM Mono',monospace",fontWeight:700}}>{meals.filter(m=>m.mealType==="cena").reduce((s,m)=>s+safeNum(m.calories),0)} kcal</div>
              </div>
              <div style={{textAlign:"center"}}>
                <div style={{fontSize:10,color:C.dim}}>Spuntini</div>
                <div style={{fontSize:12,color:C.pink,fontFamily:"'DM Mono',monospace",fontWeight:700}}>{meals.filter(m=>m.mealType==="spuntino").reduce((s,m)=>s+safeNum(m.calories),0)} kcal</div>
              </div>
            </div>
          </div>
          ...
          <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:22,padding:"18px",marginBottom:10}}>
            ...
            {quickVisible && (
              <div style={{marginBottom:12}}>
                <div style={{fontSize:11,color:C.dim,marginBottom:6}}>Suggerimenti per {MEAL_META[mealType].label.toLowerCase()}:</div>
            ...
            <textarea ref={taRef} value={input} onChange={e=>setInput(e.target.value)}
              ...
              placeholder={`Descrivi la tua ${MEAL_META[mealType].label.toLowerCase()}…\nes. "pasta al pomodoro 180g, parmigiano 20g"`}
              ...
              onFocus={e=>e.target.style.borderColor=C.teal} onBlur={e=>e.target.style.borderColor=C.border}
            />
            <button onClick={()=>analyzeMeal()} disabled={loading||!input.trim()} style={{marginTop:10,width:"100%",background:loading||!input.trim()?C.dim:`linear-gradient(135deg,${C.teal},${C.purple})`,border:"none",borderRadius:13,color:loading||!input.trim()?C.muted:"#07080f",padding:"14px",fontSize:15,fontWeight:700,cursor:loading||!input.trim()?"not-allowed":"pointer",fontFamily:"inherit",transition:"all .2s",display:"flex",alignItems:"center",justifyContent:"center",gap:8}}>
              {loading?<><Spinner/> Analisi IA…</>:"🔍 Analizza con AI"}
            </button>
          </div>
          ...
          {meals.length>0 ? (
            ...
            <div style={{textAlign:"center",padding:"44px 20px",color:C.dim}}>
              ...
              <p style={{margin:"5px 0 0",fontSize:12,color:C.dim}}>Usa i suggerimenti rapidi o descrivi il pasto</p>
            </div>
          )}
        </>}
        ...
        {tab==="storico" && <>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
            ...
            {streak>0 && <span style={{fontSize:13,color:C.amber,fontWeight:700}}>🔥 Streak: {streak} giorni</span>}
          </div>
          {histDays.length===0?(
            <div style={{textAlign:"center",padding:"50px",color:C.dim}}>
            ...
            const pct=safePct(dc,kcalGoal);
            const over=dc>kcalGoal;
